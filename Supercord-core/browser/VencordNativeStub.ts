...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Supercord/Supercord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("SupercordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("SupercordSettings", JSON.stringify(s)),
