...
While it was primarily created for Supercord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @supercord/discord-types
yarn add -D @supercord/discord-types
pnpm add -D @supercord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@supercord/discord-types";
...
```ts
import { ApplicationCommandType } from "@supercord/discord-types/enums";
