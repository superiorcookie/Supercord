...
/**
 * re-applies the supercord patch to a freshly installed discord host
...
interface DiscordUpdaterModule {
    ...
    __supercordTryInitWrapped?: boolean;
}
...
interface DiscordDesktopCore {
    ...
    __supercordStartupWrapped?: boolean;
}

const error = (...args: unknown[]) => console.error("[Supercord:HostUpdate]", ...args);
...
const retainSupercord = (updater: DiscordHostUpdater, reason: string) => {
    try {
...
const attachToUpdater = (updater: DiscordHostUpdater | null | undefined) => {
    ...
    updater.on?.("host-updated", () => retainSupercord(updater, "host-updated"));
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options?: StartCurrentVersionOptions) => {
            bound(options);
            try { retainSupercord(updater, "startCurrentVersionSync"); } catch (e) { error(e); }
        };
    }
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions?: object, options?: StartCurrentVersionOptions) => {
            ...
            try { retainSupercord(updater, "startCurrentVersion"); } catch (e) { error(e); }
        };
    }
};

const wrapStartup = (coreExports: DiscordDesktopCore | null | undefined) => {
    if (!coreExports?.startup || coreExports.__supercordStartupWrapped) return;
    coreExports.__supercordStartupWrapped = true;
    ...
    coreExports.startup = function (opts, ...rest) {
        try {
            ...
            } else if (typeof updaterModule?.tryInitUpdater === "function" && !updaterModule.__supercordTryInitWrapped) {
                /*
                 * updater not yet constructed at startup time. wrap the
                 * factory so we attach once vanilla creates it.
                 */
                updaterModule.__supercordTryInitWrapped = true;
