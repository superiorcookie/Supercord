...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("supercord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("supercord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Supercord] Installed React Developer Tools"))
                    .catch(err => console.error("[Supercord] Failed to install React Developer Tools", err));
        } catch { }
