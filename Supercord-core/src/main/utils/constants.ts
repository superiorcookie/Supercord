...
export const DATA_DIR = process.env.SUPERCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "SupercordData", suffix)
        : join(app.getPath("userData"), "..", "Supercord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Supercord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
