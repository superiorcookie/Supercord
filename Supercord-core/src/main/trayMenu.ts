...
function createSupercordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Supercord",
            submenu: [
                {
                    label: "About Supercord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Supercord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    ...
    Menu.buildFromTemplate = function (template: MenuItemConstructorOptions[]) {
        const alreadyPatched = template.some(item => item.label === "Supercord");
        if (isTrayMenu(template) && !alreadyPatched) {
            const insertIndex = findInsertIndex(template);
            const supercordItems = createSupercordMenuItems();
            template.splice(insertIndex, 0, ...supercordItems);
        }
