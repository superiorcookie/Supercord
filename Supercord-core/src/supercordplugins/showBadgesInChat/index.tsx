...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "SupercordDonor":
            return (
                <span style={{ order: settings.store.SupercordDonorPosition }}>
                    {badges.getSupercordDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "SupercordContributer":
            return isSupercordPluginDev(author.id) ? (
                <span style={{ order: settings.store.SupercordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="Supercord Contributor"
                        size={20}
                        src={"https://supercord.org/assets/favicon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showSupercordDonor && <CheckBadge badge={"SupercordDonor"} author={author} />}
            {settings.store.showSupercordContributor && <CheckBadge badge={"SupercordContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, SupercordDevs.KrystalSkull],
