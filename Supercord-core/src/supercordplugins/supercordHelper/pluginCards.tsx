...
import { isSupercordGuild, isSupercordSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Supercord. Try updating!";
    ...
    if (required) {
        const tooltipText = p.required || !dependents.length
            ? "This plugin is required for Supercord to function."
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://supercord.org/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isSupercord = isSupercordGuild(message.channel_id) && isSupercordSupport(message.author.id);
        if (!isSupercord) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://supercord.org/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
