...
export const isSupercordPluginDev = (id: string) => Object.hasOwn(SupercordDevsById, id);
export const shouldShowSupercordContributorBadge = (id: string) => isSupercordPluginDev(id) && SupercordDevsById[id].badge !== false;

export const isAnyPluginDev = (id: string) => Object.hasOwn(VencordDevsById, id) || Object.hasOwn(SupercordDevsById, id);
...
export function isSupercordGuild(id: string | null | undefined, isGuildId: boolean = false): boolean {
    if (!id) return false;
...
export function isSupercordSupport(userId: string | null | undefined): boolean {
    if (!userId) return false;

    const member = GuildMemberStore.getMember(GUILD_ID, userId);
    if (!member) return false;
    return member.roles.includes(SUPERCORD_HELPERS) || false;
}
