...
import { platformName } from "@supercordplugins/supercordHelper/utils";
import customIdle from "@plugins/customIdle";
import { gitHash, gitHashShort } from "@shared/vencordUserAgent";
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, SUPERCORD_TEAM, GUILD_ID, SUPPORT_CHANNEL_ID, SUPPORT_CHANNEL_IDS, VC_CONTRIB_ROLE_ID, VC_DONOR_ROLE_ID, VC_GUILD_ID, VC_REGULAR_ROLE_ID, VENCORD_CONTRIB_ROLE_ID } from "@utils/constants";
import { sendMessage } from "@utils/discord";
import { Logger } from "@utils/Logger";
import { Margins } from "@utils/margins";
import { isAnyPluginDev, isSupercordGuild, isSupercordSupport, isKnownIssuesCategory, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    SUPERCORD_TEAM, // Supercord Team
    DONOR_ROLE_ID, // Supercord Donor
    CONTRIB_ROLE_ID, // Supercord Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Supercord/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Supercord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Supercord/Supercord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Supercord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of Supercord, which we do not provide support for!</Paragraph>

                <Paragraph className={Margins.top8}>
                    We only provide support for <Link href="https://supercord.org/download">official builds</Link>.
                    Either <Link href="https://supercord.org/download">switch to an official build</Link> or figure your issue out yourself.
                </Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "supercord-debug",
            description: "Send Supercord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isSupercordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "supercord-plugins",
            description: "Send Supercord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isSupercordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of Supercord! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated Supercord version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://supercord.org/download">officially supported version of Supercord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        const supercordSupport = isSupercordSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && supercordSupport))
        ...
        if (supercordSupport && isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/supercord-debug") || props.message.content.includes("/supercord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
                        variant="secondary"
                    ...
                    >
                        Run /supercord-plugins
                    </Button>
                );
            }
        }

        if (supercordSupport || (isSupportChannel(props.channel.id) || isKnownIssuesCategory(props.channel.parent_id))) {
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Supercord & Vencord plugin developers for support!
                <br />
